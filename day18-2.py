...
def concat(inArr1, inArr2):
    arr1=deepcopy(inArr1)
    arr2=deepcopy(inArr2)
...
for i in puzzle:
    ...
    flattenedPuzzle.append(toBeAdded)
# print(flattenedPuzzle)
maxResult=-1
for i in range(len(flattenedPuzzle)):
    for j in range(len(flattenedPuzzle)):
        if i==j:
            pass
        result=calculation(calculate(concat(flattenedPuzzle[i],flattenedPuzzle[j])))
        if maxResult==-1 or result[0].val>maxResult:
            maxResult=result[0].val
print(maxResult)