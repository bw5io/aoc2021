...
while True:
    ...
    elif abs(left[0]-right[0])==abs(left[1]-right[1]):
        # print(2)
        # print(coordinates)
        xstep=1 if left[0]<right[0] else -1
        ystep=1 if left[1]<right[1] else -1
        x=left[0]
        y=left[1]
        for i in range(abs(left[0]-right[0])+1):
            passedRoutes, overlapCounts = validateOverlap(x, y, passedRoutes, duplicate)
            # print(x,y)
            x+=xstep
            y+=ystep    
    # print(coordinates)
